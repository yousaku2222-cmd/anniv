...
/// Whether one specific extra background colour can be applied without
/// watching an ad — either the user already unlocked that colour (watched
/// once) or bought "remove ads". [argb] is the colour's `toARGB32()` value.
final colorUnlockedProvider = Provider.family<bool, int>((ref, argb) {
  final s = ref.watch(settingsProvider);
  return s.adRemoved || s.unlockedColorValues.contains(argb);
});

/// Whether every extra colour in `AnnivEventColors.extra` has already been
/// unlocked (so the editor never needs to show a "watch an ad" prompt).
final allColorsUnlockedProvider = Provider<bool>((ref) {
  final s = ref.watch(settingsProvider);
  if (s.adRemoved) return true;
